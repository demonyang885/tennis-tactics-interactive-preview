...
test("starter-state detection follows the ready-to-serve structure instead of draft identity", () => {
  const starter = createStarterBoard("自定义名称");
  expect(isStarterBoardState(starter)).toBe(true);
  expect(isStarterBoardState({
    ...starter,
    id: "renamed-starter-id",
    title: "赛前发球套路",
    updatedAt: "2000-01-01T00:00:00.000Z",
  })).toBe(true);

  const me = starter.actors.find((actor) => actor.label === "我方")!;
  const moved = structuredClone(starter) as BoardDocument;
  moved.frames[0].poses[me.id] = [.61, .93];

  const marked = structuredClone(starter) as BoardDocument;
  marked.frames[0].marks.push({ id: "target", kind: "target", position: [.5, .2] });

  const wrongCursor = structuredClone(starter) as BoardDocument;
  wrongCursor.smartRally = { ...wrongCursor.smartRally!, phase: "move", actorId: me.id };

  expect(isStarterBoardState({ ...starter, sourceTacticId: "serve-wide" })).toBe(false);
  expect(isStarterBoardState({ ...starter, drillId: "serve-drill" })).toBe(false);
  expect(isStarterBoardState({ ...starter, smartRally: undefined })).toBe(false);
  expect(isStarterBoardState(moved)).toBe(false);
  expect(isStarterBoardState(marked)).toBe(false);
  expect(isStarterBoardState(wrongCursor)).toBe(false);
  expect(isStarterBoardState(addFrame(starter, 0))).toBe(false);
});

test("restoring a worked board preserves its draft identity and rebuilds the canonical starter", () => {
  let worked = createStarterBoard("保留这个名称");
  const originalId = worked.id;
  const ball = worked.actors.find((actor) => actor.kind === "ball")!;
  worked = setPath(worked, 0, {
    id: "worked-shot",
    kind: "shot",
    actorId: ball.id,
    from: worked.frames[0].poses[ball.id],
    to: [.72, .24],
    control: [.81, .54],
  });
  worked = addMark(worked, 0, { id: "worked-target", kind: "target", position: [.72, .24] });
  ...
  const beforeRestore = structuredClone(worked) as BoardDocument;

  const restored = restoreStarterBoard(worked);

  expect(worked).toEqual(beforeRestore);
  ...
  expect(isStarterBoardState(restored)).toBe(true);
  expect(validateBoardDocument(restored)).toEqual({ ok: true, value: restored });

  const me = restored.actors.find((actor) => actor.label === "我方")!;
  const opponent = restored.actors.find((actor) => actor.label === "对手")!;
  const restoredBall = restored.actors.find((actor) => actor.kind === "ball")!;
  ...
  expect(restored.frames[0].poses[me.id]).toEqual([.64, .98]);
  expect(restored.frames[0].poses[opponent.id]).toEqual([.30, .07]);
  expect(restored.frames[0].poses[restoredBall.id]).toEqual([.64, .96]);
  expect(restored.smartRally).toEqual({
    version: 2,
    frameId: restored.frames[0].id,
    phase: "shot",
    hitterId: me.id,
    actorId: restoredBall.id,
  });
});

test("restoring an unchanged starter is idempotent", () => {
  const starter = createStarterBoard("无需还原");
  expect(restoreStarterBoard(starter)).toBe(starter);
});
