...
> 發布後狀態：本文件保留發布前的整理過程。v0.2.0 其後已固定於 `9158a2b`、推送至 main，並於 2026-09-19 完成 GitHub Pages 部署。正式收尾證據見 [V0_2_RELEASE_CLOSEOUT.md](./V0_2_RELEASE_CLOSEOUT.md)。
...
整理階段建立的版本標籤為 `v0.2.0`；其後已推送遠端，並指向發布提交 `9158a2b`。用 `git rev-parse v0.2.0^{commit}` 可核對固定提交。
除原有未追蹤 QA 資料外，本次程式、測試與現行文件均納入版本提交；後續推送與部署結果記錄在發布收尾文件。

## 發布邊界與後續

v0.2.0 已合入 main 並通過 CI／Pages 部署；線上 version.json 已核對為 version 0.2.0、commit `9158a2b`。後續變更仍須走同倉庫 PR 與 CI。
