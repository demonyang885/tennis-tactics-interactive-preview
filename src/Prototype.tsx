...
const BOARD_MARK_NAMES:Record<BoardMark["kind"],string>={target:"目标区",cone:"标志碟",basket:"球篮",text:"文字提示",freehand:"手绘线"};
...
function BoardCanvas({board,frameIndex,selection,setSelection,tool,actorPreset,pathKind,markPreset,curved,previewing,elapsed,preview,commit,finishPreview,onComplete,onNudge,onDelete,onError}:{
  ...
  return <div ref={holderRef} className="board-canvas" data-testid="board-canvas" data-scroll-drag="ignore" tabIndex={0} role="application" aria-label="可编辑网球战术画板。使用下方选择、球员、球路、跑位和标记工具；选中对象后可用方向键微调，Delete 键删除。" onKeyDown={onKeyDown} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={event=>endPointer(event)} onPointerCancel={event=>endPointer(event,true)}><canvas ref={canvasRef}/></div>;
}
...
function BoardEditor({ initialBoard, back, openLibrary }:{initialBoard:BoardDocument;back:()=>void;openLibrary:()=>void}) {
  ...
  const [filesOpen,setFilesOpen]=useState(false),[frameOpen,setFrameOpen]=useState(false),[objectsOpen,setObjectsOpen]=useState(false),[drillOpen,setDrillOpen]=useState(false),[objectOpen,setObjectOpen]=useState(false),[toolPalette,setToolPalette]=useState<"actors"|"marks"|null>(null);
  ...
  const editorRef=useRef<HTMLDivElement>(null),selectToolRef=useRef<HTMLButtonElement>(null),sheetOpenerRef=useRef<HTMLElement|null>(null),sheetFocusTimerRef=useRef<number|null>(null),importRef=useRef<HTMLInputElement>(null),boardRef=useRef(board),committedBoardRef=useRef(board),needsSaveRef=useRef(true);boardRef.current=board;
  ...
  const completeCanvasAction=(nextSelection:BoardSelection)=>setSelection(nextSelection);
  ...
  const chooseSelectionTool=()=>{if(tool!=="select"){chooseTool("select");return;}setObjectsOpen(true);};
  ...
  const toolStatus=tool==="actor"?`点球场放置${actorPreset==="me"?"我方球员":actorPreset==="opponent"?"对手球员":"网球"}`:tool==="shot"?`从网球拖到落点 · ${pathKind==="feed"?"喂球":"球路"}${curved?"曲线":"直线"}`:tool==="move"?`从球员拖到跑位终点 · ${curved?"曲线":"直线"}`:tool==="mark"?`点球场放置${markPreset==="target"?"目标区":markPreset==="cone"?"标志碟":markPreset==="basket"?"球篮":markPreset==="text"?"文字":"手绘线"}`:"";
  return <div ref={editorRef} className={`board-editor ${previewing?"is-previewing":"is-editing"}`}>
    <div className="board-canvas-shell">
      <div className="board-canvas-meta">{!previewing&&tool!=="select"?<><span>{tool==="actor"?"球员":tool==="shot"?"球路":tool==="move"?"跑位":"标记"}</span><strong aria-live="polite">{toolStatus}</strong></>:!previewing&&selection?<><span>已选</span><strong aria-live="polite">{selectedLabel||"场上对象"}</strong><button aria-label={`调整${selectedLabel||"场上对象"}`} onClick={event=>{rememberSheetOpener(event.currentTarget);openSelectedObject();}}><Pencil2Icon/>调整</button></>:<><span>{previewing?`预览 · 第 ${currentPlaybackFrame+1} 拍`:`第 ${frameIndex+1} 拍`}</span><strong>{board.frames[currentPlaybackFrame]?.label}</strong>{drill&&<button onClick={event=>{rememberSheetOpener(event.currentTarget);setIsPlaying(false);setDrillOpen(true);}}><TargetIcon/>练到场上</button>}</>}</div>
    ...
    {previewing?<div className="board-playback-dock">
      ...
      <div className="board-frame-rail"><Carousel className="board-frame-carousel" contentClassName="board-frame-track" ariaLabel="战术拍次">{board.frames.map((item,index)=><button key={item.id} className={index===frameIndex?"is-active":""} aria-label={index===frameIndex?`当前拍次：${item.label}，点按编辑`:`打开第 ${index+1} 拍：${item.label}`} aria-pressed={index===frameIndex} onClick={event=>{if(index===frameIndex){rememberSheetOpener(event.currentTarget);openFrameSheet();return;}setFrameIndex(index);setSelection(null);setTool("select");}}><span>{index+1}</span><strong>{item.label}</strong></button>)}</Carousel><button className="board-add-frame" aria-label="新建下一拍" onClick={()=>addNextFrame(false)}><PlusIcon/><span>下一拍</span></button></div>
      <nav className="board-edit-dock" aria-label="画板编辑工具">
        <div className="board-tool-rail" role="toolbar" aria-label="画板工具">
          <button ref={selectToolRef} className={tool==="select"?"is-active":""} aria-label="选择" aria-pressed={tool==="select"} aria-haspopup={tool==="select"?"dialog":undefined} aria-expanded={tool==="select"?objectsOpen:undefined} onClick={event=>{if(tool==="select")rememberSheetOpener(event.currentTarget);chooseSelectionTool();}}><CursorArrowIcon/><span>选择</span></button>
          <button className={tool==="actor"?"is-active":""} aria-label="球员" aria-pressed={tool==="actor"} aria-haspopup="dialog" aria-expanded={toolPalette==="actors"} onClick={event=>{rememberSheetOpener(event.currentTarget);setToolPalette("actors");}}><PersonIcon/><span>球员</span></button>
          <button className={tool==="shot"?"is-active":""} aria-label="球路" aria-pressed={tool==="shot"} onClick={()=>{setPathKind("shot");chooseTool("shot");}}><ArrowTopRightIcon/><span>球路</span></button>
          <button className={tool==="move"?"is-active":""} aria-label="跑位" aria-pressed={tool==="move"} onClick={()=>chooseTool("move")}><CornerTopRightIcon/><span>跑位</span></button>
          <button className={tool==="mark"?"is-active":""} aria-label="标记" aria-pressed={tool==="mark"} aria-haspopup="dialog" aria-expanded={toolPalette==="marks"} onClick={event=>{rememberSheetOpener(event.currentTarget);setToolPalette("marks");}}><DrawingPinIcon/><span>标记</span></button>
        </div>
        <button className="board-primary-play" aria-label={`播放战术，${board.frames.length} 拍，共 ${totalDuration.toFixed(1)} 秒`} disabled={totalDuration<=0} onClick={startPlayback}><PlayIcon/><span>播放战术</span></button>
      </nav>
    </>}

    <BottomSheet open={toolPalette!==null} onOpenChange={open=>{if(!open){keyboard.hide();setToolPalette(null);restoreSheetFocus("opener");}}} title={toolPalette==="actors"?"添加球员或网球":"标记与更多"} description={toolPalette==="actors"?"选好角色，再点球场放置。":"目标、器材与教练喂球都收在这里。"} snap={toolPalette==="actors"?.52:.72}><div className="board-sheet"><div className="board-file-actions">
      {toolPalette==="actors"?<><button onClick={()=>chooseAdd("actor","me")}><PersonIcon/><span><strong>我方球员</strong><small>点球场放置</small></span></button><button onClick={()=>chooseAdd("actor","opponent")}><PersonIcon/><span><strong>对手球员</strong><small>点球场放置</small></span></button><button onClick={()=>chooseAdd("actor","ball")}><ComponentInstanceIcon/><span><strong>网球</strong><small>点球场放置</small></span></button></>:<><button onClick={()=>chooseAdd("mark","target")}><TargetIcon/><span><strong>目标区</strong><small>落点范围</small></span></button><button onClick={()=>chooseAdd("mark","cone")}><DrawingPinIcon/><span><strong>标志碟</strong><small>场上器材</small></span></button><button onClick={()=>chooseAdd("mark","basket")}><ArchiveIcon/><span><strong>球篮</strong><small>喂球位置</small></span></button><button onClick={()=>chooseAdd("mark","text")}><TextIcon/><span><strong>文字提示</strong><small>教练口令</small></span></button><button onClick={()=>chooseAdd("mark","freehand")}><Pencil2Icon/><span><strong>手绘线</strong><small>拖动画线</small></span></button><button onClick={()=>{setPathKind("feed");chooseTool("shot");setToolPalette(null);restoreSheetFocus("canvas");}}><ResumeIcon/><span><strong>喂球路线</strong><small>从网球拖到落点</small></span></button></>}
    </div></div></BottomSheet>
    <BottomSheet open={filesOpen} onOpenChange={open=>{setSheetVisibility(setFilesOpen,open);if(!open)restoreSheetFocus("opener");}} title="画板文件" description="保存在本机，也可带走 PNG 或完整 JSON。" snap={.78}><div className="board-sheet">
